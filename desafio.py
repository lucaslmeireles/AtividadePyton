...
class Sheets():
    ...
    def build(self):
        #Code available in the googlespreadsheet API documentation in https://developers.google.com/sheets/api/quickstart/python?hl=pt-br
        creds = None
        ...
        try:
            #With the authentication process is done, then the service will be initialized
    ...
    def update_state(self,range, value):
        try:
            result = (
    ...
    #Function that update the minimum value to be approved in the final exam, for any other student the default value is 0
    def final_note(self, range, value={"values": [[0]]}):
        try:
            result = (
                 self.service.spreadsheets()
                .values()
    ...
    def iterate(self):
        for row in self.values:
            ...
            if attendance_percentage < 75:
                ...
                #The update_state function is called and passed the range and the value to be updated
                self.update_state(range,{"values": [['Reprovado por falta']]})
                self.final_note(range)
            elif (avg < 5):
                print(f"The student {row[0]} was reproved by avarage")
                self.update_state(range,{"values": [['Reprovado por média']]})
                self.final_note(range)
                ...
                self.update_state(range,{"values": [['Exame final']]})
                #The formula calculates how much the student needs to pass the final exam
                self.final_note(range,{"values": [[math.ceil(avg*10 - (2 * 5))]]})
            else:
                #If the student get any avarage above 7 he is approved	
                print(f"The student {row[0]} was approved")  
                self.update_state(range,{"values": [['Aprovado']]})
                self.final_note(range)
